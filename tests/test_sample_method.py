...
class SampleMethodTestCase(unittest.TestCase):
    ...
    def test_sample_returns_state_dict(self):
        ...
        expected_keys = ["u", "x", "logl", "assignments", "blobs", "iter", 
                        "calls", "steps", "efficiency", "ess", "accept", 
                        "beta", "logz"]
    ...
    def test_sample_increments_iteration(self):
        ...
        sampler.run(n_total=128)
        iter_before = sampler.iter
        
        state = sampler.sample()
        
        # Iteration should have incremented
        self.assertEqual(sampler.iter, iter_before + 1)
    ...
    def test_sample_increments_calls(self):
        ...
        self.assertGreater(sampler.calls, calls_before)
        self.assertEqual(state["calls"], sampler.calls)
    ...
    def test_run_uses_sample_method(self):
        ...
        self.assertIsNotNone(sampler.particles.get("x", flat=True))
        self.assertGreater(sampler.iter, 0)
        self.assertGreater(sampler.calls, 0)
        self.assertEqual(sampler.beta, 1.0)  # Should reach beta=1
    ...
    def test_manual_sampling_loop(self):
        ...
        self.assertEqual(sampler.iter, initial_iter + n_iterations)
    ...
    def test_sample_particles_update(self):
        ...
        particles_before = sampler.particles.get("x", flat=True).copy()
        
        # Run sample
        state = sampler.sample()
        
        # Get particles after sample
        particles_after = sampler.particles.get("x", flat=True)
    ...
    def test_sample_state_consistency(self):
        ...
        np.testing.assert_array_equal(state["u"], sampler.u)
        np.testing.assert_array_equal(state["x"], sampler.x)
        np.testing.assert_array_equal(state["logl"], sampler.logl)
        self.assertEqual(state["iter"], sampler.iter)
        self.assertEqual(state["calls"], sampler.calls)
        self.assertEqual(state["beta"], sampler.beta)
        self.assertEqual(state["logz"], sampler.logz)
