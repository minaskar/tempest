...
class EdgeCasesTestCase(unittest.TestCase):
    ...
    def log_likelihood_bimodal(x):
        ...
        return (
            max_logp
            + np.log(np.exp(logp1 - max_logp) + np.exp(logp2 - max_logp))
            - np.log(2)
        )
    ...
    def test_small_n_particles(self):
        """Test with very small n_particles."""
        n_dim = 2
        sampler = Sampler(
            prior_transform=self.prior_transform_gaussian,
            log_likelihood=self.log_likelihood_gaussian,
        ...
        self.assertEqual(sampler.n_particles, 8)
