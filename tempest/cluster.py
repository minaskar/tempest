...
class GaussianMixture:
    ...
    def __init__(self, n_components=1, covariance_type='full', max_iter=1000, 
                 n_init=1, tol=1e-3, reg_covar=1e-6, random_state=None):
    ...
    def fit(self, X, sample_weight=None):
        ...
        for init in range(self.n_init):
            ...
            for iteration in range(self.max_iter):
                ...
                new_lower_bound = self._compute_lower_bound(X, weights, means, covariances, sample_weight)
    ...
    def _initialize_parameters(self, X, sample_weight):
        ...
        for k in range(1, self.n_components):
            ...
            distances = np.min([np.sum((X - means[j])**2, axis=1) for j in range(k)], axis=0)
    ...
    def _m_step(self, X, responsibilities, sample_weight):
        ...
        # Update means
        means = np.dot(weighted_resp.T, X) / (np.sum(weighted_resp, axis=0)[:, np.newaxis] + 1e-10)
    ...
    def _compute_covariances(self, X, means, weighted_resp):
        ...
        if self.covariance_type == 'full':
            ...
            for k in range(self.n_components):
                diff = X - means[k]
                covariances[k] = np.sum(weighted_resp[:, k, np.newaxis] * diff**2, axis=0)
                covariances[k] /= (np.sum(weighted_resp[:, k]) + 1e-10)
    ...
    def predict(self, X):
        ...
        for k in range(self.n_components):
            ...
            try:
                log_probabilities[:, k] = np.log(self.weights_[k] + 1e-10) + \
                    multivariate_normal.logpdf(X, mean=self.means_[k], 
    ...
    def bic(self, X):
        ...
        n_parameters = (self.n_components - 1) + self.n_components * n_features + cov_params
        
        # Log-likelihood
        log_likelihood = self._compute_lower_bound(
...
class HierarchicalGaussianMixture:
    ...
    def __init__(self, n_init=1, max_iterations=1000, min_points=None, threshold_modifier=1.0,
                 covariance_type='full', verbose=False, normalize=False):
        self.n_init = n_init
    ...
    def fit(self, X, sample_weight=None):
        ...
        for cluster_idx, indices in enumerate(clusters):
            ...
            if len(data) >= n_features:
                ...
                center = gmm.means_[0]
                cov = gmm.covariances_[0] if self.covariance_type == 'full' else gmm.covariances_
            else:
        ...
        self.cluster_weights_ = np.array([
            np.sum(sample_weight[labels == i]) / total_weight for i in range(self.n_clusters_)
        ])
    ...
    def predict(self, X):
        ...
        if self._gmm_ready:
            try:
                ...
                if self.verbose:
                    print(f"Warning: GMM prediction failed: {exc}. Using nearest center fallback.")
    ...
    def predict_proba(self, X):
        ...
        if self._gmm_ready:
            try:
                ...
                if self.verbose:
                    print(f"Warning: GMM probability prediction failed: {exc}. Using distance fallback.")
        ...
        for k in range(self.n_clusters_):
            ...
            if self.covariance_type == 'full':
                try:
                    ...
                    log_prob = multivariate_normal.logpdf(X, mean=mean, cov=regularized_cov)
                except Exception:
                    log_prob = multivariate_normal.logpdf(X, mean=mean, cov=np.eye(len(mean)))
            else:
                try:
